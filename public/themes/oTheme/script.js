...
jQuery(document).ready(function () {
    ...
    jQuery.get(moo_RestUrl + "moo-clover/v1/theme_settings/oTheme", function (data) {
        ...
        window.showMore = data.settings.oTheme_show_more_button === 'on';
        window.showCategoryIcon = data.settings.oTheme_show_category_icon === 'on';
        window.showSearch = data.settings.oTheme_showSearchSection === 'on';
        window.showTopPicks = data.settings.oTheme_showTopPicksSection === 'on';
        ...
        if (sooStoreOptions && sooStoreOptions.modifiersSettings){
            window.modifiers_settings = sooStoreOptions.modifiersSettings;
            osnOnlineStoreBase();
        } else {
...
function osnOnlineStoreBase() {
    ...
    content += '</div>';
    jQuery('#moo_OnlineStoreContainer').html(content).promise().done(function () {
        osnLaunchLoading();
        osnLoadCatItemData();
    });
            ...
            if (data.items.length != 0) {
                data.items.forEach(function (item) {
                    ...
                    contentItems += '</div>'
                        ...
                        + ' <h4>' + item.name + '</h4>'
                        + '<div class="price-product"><p>$' + parseFloat(item.price / 100).toFixed(2) + '</p><div class="price-menu ' + ((item.stockCount == "out_of_stock") ? 'unavailable' : '') + '" ' + ((item.stockCount != "out_of_stock") ? 'onclick="osnItemDescriptionModal(\'' + item.uuid + '\')' : 'title="This item is out of stock"') + '"><i class="' + ((item.has_modifiers) ? 'soo-icon-cog' : 'soo-icon-add_circle') + '"></i></div></div>'
                        + '</div>'
                        + '</div>';
        ...
        jQuery.get(moo_RestUrl + "moo-clover/v1/items/most_purchase",
            function (data) {
                if (data && data.items.length > 0) {
                    content += '<li class="osnMenuListCatItem"><a data-id="#topp" onclick="osnScrollToCat(this)" class="osnCatLink">' + ((window.showCategoryIcon) ? '<span><i class="soo-icon-star"></i></span><span>' : '<span style="margin:13px;">') + 'Top picks</span></a></li>';
                    ...
                    data.items.forEach(function (item) {
                        ...
                        contentItems += '</div>'
                            + '<div class="osnContentItem">'
                            + ' <h4>' + item.name + '</h4>'
                            + '<div class="price-product"><p>$' + parseFloat(item.price / 100).toFixed(2) + '</p><div class="price-menu " ' + ((item.stockCount != "out_of_stock") ? 'onclick="osnItemDescriptionModal(\'' + item.uuid + '\')' : 'title="This item is out of stock"') + '"><i class="' + ((item.has_modifiers) ? 'soo-icon-cog' : 'soo-icon-add_circle') + '"></i></div></div>'
                            + '</div>'
                            + '</div>';
                    });
                    contentItems += '</div>';
...
function osnRenderItems(contentItems, content) {
    ...
    jQuery.get(endpoint,
            ...
            if (data) {
                data.forEach(function (element) {
                    ...
                    if (element.items.length > 0) {
                        content += '<li class="osnMenuListCatItem"><a data-id="#cat-' + element.uuid + '" onclick="osnScrollToCat(this)" class="osnCatLink">' + ((window.showCategoryIcon) ? '<span><i class="soo-icon-food"></i></span><span>' : '<span style="margin:13px;">') + '' + element.name + '</span></a></li>';
                        ...
                        element.items.forEach(function (item) {
                            ...
                            contentItems += '</div>'
                                + '<div class="osnContentItem">'
                                + ' <h4>' + item.name + '</h4>'
                                + '<div class="price-product"><p>$' + parseFloat(item.price / 100).toFixed(2) + '</p><div class="price-menu ' + ((!element.available || item.stockCount == "out_of_stock") ? 'unavailable' : '') + '" ' + ((element.available && (item.stockCount != "out_of_stock")) ? 'onclick="osnItemDescriptionModal(\'' + item.uuid + '\')' : 'title="This item is ' + ((!element.available) ? 'not available' : 'out of stock') + '"') + '"><i class="' + ((item.has_modifiers) ? 'soo-icon-cog' : 'soo-icon-add_circle') + '"></i></div></div>'
                                + '</div>'
                                + '</div>';
                        });
                        if (window.showMore && element.items.length == 5) {
                            contentItems += '<div id="osnMoreItems' + element.uuid + '" class="osnCardItem osnMoreItems" onclick="osnGetMoreItems(\'' + element.uuid + '\')" >'
                                + '<div><i class="soo-icon-angle-right"></i></div>'
        ...
        function (data) {
            content += '</ul>';
            content += '</div>'
            content += '<span id="OsnPanScrollLeft" onmouseleave="osnOnMouseLeave(this)" onmouseenter="osnOnMouseEnter(this,\'osnMenu\')" class="panner bounce-left" data-scroll-modifier="-1"><i class="soo-icon-arrow-left-alt1"></i></span>';
        ...
        if (data != null && data.items != null && data.items.length > 0) {
            ...
            data.items.forEach(function (item) {
                ...
                contentItems += '</div>'
                    ...
                    + '</p><div class="price-menu ' + ((!data.available || item.stockCount == "out_of_stock") ? 'unavailable' : '') + '" ' + ((data.available && (item.stockCount != "out_of_stock")) ? 'onclick="osnItemDescriptionModal(\'' + item.uuid + '\')' : 'title="This item is ' + ((!data.available) ? 'not available' : 'out of stock') + '"') + '"><i class="' + ((item.has_modifiers) ? 'soo-icon-cog' : 'soo-icon-add_circle') + '"></i></div></div>'
                    + '</div>'
                    + '</div>';
            });
        }
...
function osnShowHideCart() {
    ...
    if (jQuery('#osnCartModal').length) {
        ...
        html += '<span>your cart</span><div><button class="cartModifierBtn" onclick="osnClickEditBtnCart()">edit</button><div class="osnCloseCart soo-icon-close" onclick="osnShowHideCart()"></div></div>';
        html += '</div>';
        ...
        if (data != undefined && Object.keys(data.items).length > 0) {
            ...
            jQuery.each(data.items, function (index, item) {
                ...
                if (!window.moo_theme_setings
                    ...
                    html += '<div class="osnCartEditButton osnCartEditBtn" onclick="osnUpdateSpecialInsinCart(\'' + index + '\',\'' + item.special_ins + '\')"><i class="soo-icon-pencil"></i></div>';
                }
                if (item.modifiers.length > 0) {
                    html += '<div class="osnCartEditButton osnCartModifiersBtn" onclick="osnUpdateModifier(\'' + index + '\')"><i class="soo-icon-cog"></i></div>';
                }
                html += '<div class="osnCartEditButton osnCartDeleteBtn" data-item="' + index + '" onclick="osnOnDeletFromCart(this)"><i  class="soo-icon-cancel"></i></div>';
                html += '</div>';
                priceItem = 0;
            });
...
function osnUpdatePriceCart() {
    jQuery.get(moo_RestUrl + "moo-clover/v1/cart", function (data) {
        ...
        if (!data.totals) jQuery('#osnCartItems').html('<p class="osnEmptyCart">Your cart is empty!</p>');
    });
}
...
function osnItemDescriptionModal(item_uuid) {
    if (jQuery('#osnItemModal').length == 0) {
        osnLaunchLoading();
        osnItemQty = 1;
        ...
        jQuery.get(moo_RestUrl + "moo-clover/v1/items/" + item_uuid, function (data) {
            ...
            content += '<span class="soo-icon-cancel" onclick="osnItemDescriptionModalClose()"></span>';
            content += '</div>';
            ...
            content += '<div id="osnChooseQuantity">'
                ...
                + '<a data-op="decrement" onclick="osnSelectQuantity(this)" class="modifier-quantity"><i class="soo-icon-minus"></i></a>'
                + '<span id="osnItemQuantity" class="product-quantity-wrapper"><span class="product-quantity-times">x</span>' + osnItemQty + '</span>'
                + '<a onclick="osnSelectQuantity(this)"  data-op="increment" class="modifier-quantity"><i class="soo-icon-plus"></i></a>'
                + '</div>'
                + '</div>';
...
function osnItemModalModifier(item_uuid, index) {
    ...
    jQuery.get(moo_RestUrl + "moo-clover/v1/items/" + item_uuid, function (data) {
        ...
        if (index != null) {
            ...
            osn_Item_Modifers.forEach(group => {
                group.modifiers.forEach(modifier => {
                    if (uuids.indexOf(modifier.uuid) !== -1) {
                        OsnSelectModifier(group.uuid, modifier.uuid);
                        let cartModifier = osnCart.items[index].modifiers.find(function (value) {
                            return value.uuid === modifier.uuid
                        });
                        if (cartModifier) {
        ...
        content += '<span class="soo-icon-close" onclick="osnCloseModalModifier()"></span>';
        content += '</div>';
        ...
        if (osn_Item_Modifers.length > 0) {
            ...
            if (osn_Item_Modifers.length > 1) {
                ...
                content += '</ul>';
                content += '</div>';
                content += '<span id="OsnPanScrollLeft" onmouseleave="osnOnMouseLeave(this)" onmouseenter="osnOnMouseEnter(this,\'osnMenuMG\')" class="panner bounce-left" data-scroll-modifier="-1"><div><i class="soo-icon-arrow-left-alt1"></i></div></span>';
                content += '<span id="OsnPanScrollRight" onmouseleave="osnOnMouseLeave(this)" onmouseenter="osnOnMouseEnter(this,\'osnMenuMG\')" class="panner bounce-right" data-scroll-modifier="1"><div><i class="soo-icon-arrow-right-alt1"></i></div></span>';
                content += '<span onclick="osnCloseModalModifier()" class="panner closeModal"><div><i class="soo-icon-cancel"></i></div></span>';
...
function osnGetModifier(page) {
    ...
    if (!isNaN(page)) {
        ...
        osn_Item_Modifers[page].modifiers.forEach(function (modifier) {
            ...
            if (window.modifiersHasImage) content += '<div onclick="OsnSelectModifier(\'' + osn_Item_Modifers[page].uuid + '\',\'' + modifier.uuid + '\')" class="osnModifierImage"><i class="soo-icon-spoon-knife"></i></div>';
            ...
            if (window.modifiers_settings.qtyForAll) {
                if (osn_Item_Modifers[page].max_allowd == 1 && osn_Item_Modifers[page].min_required == 1) {
                    ...
                    content += '<a data-op="decrement" onclick="osnQuantitySelectedModifier(this)" data-uuid-group-modifier="' + osn_Item_Modifers[page].uuid + '" data-uuid-modifier="' + modifier.uuid + '" class="modifier-quantity"><i class="soo-icon-minus"></i></a>';
                    content += '<span class="product-quantity-wrapper"><span class="product-quantity-times">x</span>' + ((modifier.qty) ? modifier.qty : '1') + '</span>';
                    content += '<a onclick="osnQuantitySelectedModifier(this)" data-uuid-modifier="' + modifier.uuid + '" data-uuid-group-modifier="' + osn_Item_Modifers[page].uuid + '" data-op="increment" class="modifier-quantity"><i class="soo-icon-plus"></i></a>';
                    content += '</div>';
                } else {
...
function OsnQtyComment(min_required, max_allowd) {
    ...
    if (min_required != null && max_allowd != null && min_required == 1 && max_allowd == 1) {
        html += '<i class="soo-icon-notice"></i> required';
    } else {
        if (min_required != null && max_allowd != null && max_allowd == min_required) {
            html += '<i class="soo-icon-notice"></i> Must choose ' + min_required + ' options';
        } else {
            if (min_required != null && max_allowd != null && min_required >= 1 && max_allowd > 1) {
                html += '<i class="soo-icon-notice"></i> Must choose between ' + min_required + ' & ' + max_allowd + ' options';
            } else {
                if (min_required != null && min_required == 1)
                    html += '<i class="soo-icon-notice"></i> Must choose at least 1 option';

                if (min_required != null && min_required > 1)
                    html += '<i class="soo-icon-notice"></i> Must choose at least ' + min_required + ' options';

                if (max_allowd != null && max_allowd > 1)
                    html += '<i class="soo-icon-notice"></i> Select up to ' + max_allowd + ' options';

                if (max_allowd != null && max_allowd == 1)
                    html += '<i class="soo-icon-notice"></i> Select one option';
            }
        }
    }
    return html;
...
function OsnSelectModifier(Guuid, uuid) {
    ...
    group = osn_Item_Modifers.filter(function (value) {
        return value.uuid === Guuid
    })[0];
    modifier = group.modifiers.filter(function (value) {
...
function osnQuantitySelectedModifier(e) {
    ...
    group = osn_Item_Modifers.find(function (value) {
        return value.uuid === jQuery(e).data('uuid-group-modifier')
    });
    modifier = group.modifiers.find(function (value) {
        return value.uuid === jQuery(e).data('uuid-modifier')
    });
